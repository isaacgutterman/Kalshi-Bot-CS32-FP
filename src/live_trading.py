...
def run_stream(args: argparse.Namespace) -> None:
    ...
    while True:
        ...
        for snap in snapshots:
            ...
            print(f"  {snap.ticker:22s} mid={snap.mid:6.2f} avg={avg:6.2f} spread={spread_text} signal={signal}")
        print()
        iteration += 1
...
def run_paper(args: argparse.Namespace) -> None:
    ...
    while True:
        ...
        for snap in snapshots:
            ...
            print(
                f"  {snap.ticker:22s} mid={snap.mid:6.2f} avg={avg:6.2f} signal={signal:7s} "
                f"pos={books[snap.ticker].qty_yes:+3d} pnl={total:8.2f}"
            )
        print()
        iteration += 1
        if args.iterations and iteration >= args.iterations:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Live stream and paper trading for Kalshi baseball momentum strategy.")
